...
getProductById(id: string): IProduct - возвращает продукт по id.
...
В моделе происходит валидация полей формы методами: validationPhone, validationEmail, validationAddress, а также установка кнопки "Далее" неактивной.
...
### Слой Presenter
Презентер реализован в src/index.ts. Он организует связь между моделями, представлением, API и модальными окнами, обрабатывает событие и вызывает метод модели для изменения данных и их валидации, а также запускает рендер при изменении данных.
На главной странице пользователь видит список предложенных товаров Products. Когда пользователь совершает клик по карточке товара, презентер обрабатывает это событие и вызывает открытие CardPreviewModal (слой view), затем слой вью реагирует на действие пользователя по нажатию на кнопку "Купить" и генерирует событие добавления товара в корзину. Презентер обрабатывает его и вызывает метод модели для изменения данных BasketModel добавляя выбранный товар в корзину.
...
#### Интерфейсы взаимодействия с Api
В проекте реализованы три интерфейса

```TypeScript
//Интерфейс ответа сервера, содержит поле items - массив продуктов и поле total - общее количество продуктов
export interface IProductResponse {
  items: IProduct[];
  total: number;
}

//Интерфейс описывает структуру объекта продукта, получаемого с сервера
export interface IProduct {
  id: string; //идентификатор продукта
  title: string; //название продукта
  category: string; //категория продукта
  image: string; //изображение продукта
  price: number | null; //стоимость продукта, при значении null отображается как "Бесценно"
  description: string; //описание продукта
}

//Интерфейс для отправки на сервер POST-запроса с контактными данными покупателя, общей суммой совершенной покупки и выбранными товарами
...
export interface IOrder {
  ...
  address: string, //адрес получения
	...
	items: string[], //купленные продукты
}
```

### EventEmitter — брокер событий
EventEmitter реализует паттерн наблюдатель и предоставляет механизм подписки и оповещения о событиях в приложении.

**Методы:**
on - Подписка на конкретные события по имени или регулярному выражению
off - Отписка от событий
emit - Генерация событий с передачей данных
